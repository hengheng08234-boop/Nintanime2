...
// Inlined instead of imported from ../_shared/payway.ts — the Supabase
// Dashboard's "Create new edge function" editor only deploys the files
// you add in that screen, so a relative import to a sibling function's
// _shared folder fails to bundle ("Module not found"). If you deploy
// this function via the Supabase CLI from the repo instead, importing
// from ../_shared/payway.ts works fine there too — but keeping this
// copy self-contained means it deploys correctly either way.
const corsHeaders = {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: episode, error: episodeError } = await adminClient
      .from("episodes")
      .select("video_url, is_free_preview")
      .eq("id", episodeId)
      .maybeSingle();
    ...
    // Admins can always preview (to check an upload played correctly).
    // Free-preview episodes are open to any signed-in user as a trial taste.
    const hasAccess = isSubscribed || profile.is_admin === true || episode.is_free_preview === true;
